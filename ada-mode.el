...
(defcustom ada-move-to-declaration nil
  "Non-nil->`ada-move-to-start' moves to the subprog declaration, not to `begin'."
  :type 'boolean :group 'ada)
...
(if (featurep 'xemacs)
    (advice-add 'parse-partial-sexp :around
                (lambda (orig-fun from to &rest args)
...
(defvar skeleton-point nil
  "The start position of the skeleton insertion.")
  ...
  (save-excursion
    (let ((aa-end (point)))
      (ada-adjust-case-region
       (progn (goto-char skeleton-point) (forward-word-strictly -1)
              (point))
       (goto-char aa-end)))))
...
(defun ada-indent-current ()
  "Indent the current line as Ada code.
Return the calculation that was done, including the reference point
and the offset."
  (interactive)
  (let ((orgpoint (point-marker))
        cur-indent tmp-indent prev-indent)

    (unwind-protect
        (with-syntax-table ada-mode-symbol-syntax-table

          ;; Temporarily wrap parse-partial-sexp if in XEmacs
          (when (featurep 'xemacs)
            (let ((parse-partial-sexp-advice 
                   (lambda (orig-fun &rest args)
                     "Handles special character constants and gnatprep statements."
                     (apply orig-fun args))))
              (advice-add 'parse-partial-sexp :around parse-partial-sexp-advice)))
          ...
          (while tmp-indent
            (setq prev-indent (+ prev-indent 
                                 (if (numberp (car tmp-indent))
                                     (car tmp-indent)
                                   (eval (car tmp-indent)))))
            (setq tmp-indent (cdr tmp-indent)))

          ;; Only re-indent if indentation differs from the current
          (unless (= (save-excursion (back-to-indentation) (current-column)) prev-indent)
            (beginning-of-line)
      ...
      ;; Remove the temporary advice if it was applied
      (when (featurep 'xemacs)
        (advice-remove 'parse-partial-sexp 'parse-partial-sexp-advice)))
...
(when (or (<= emacs-major-version 20) (featurep 'xemacs))
  (advice-add 'comment-region :before
              (lambda (beg end &optional arg)
                "Allow uncommenting anywhere in Ada mode."
                (when (and (consp arg)  ;; A prefix with C-u is '(4), whereas
...
(defun ada-uncomment-region (beg end &optional arg)
  "Uncomment region from BEG to END.
ARG gives the number of comment characters to remove."
  (interactive "r\nP")

  ;; For older Emacs versions and XEmacs, wrap comment-region temporarily.
  (if (or (<= emacs-major-version 20) (featurep 'xemacs))
      (let ((ada-comment-region-advice
             (lambda (orig-fun &rest args)
               "Temporarily modifies comment-region behavior for older Emacs/XEmacs."
               (apply orig-fun args))))
        
        (advice-add 'comment-region :around ada-comment-region-advice)
        (comment-region beg end (- (or arg 2)))
        (advice-remove 'comment-region ada-comment-region-advice))
    
    ;; Modern Emacs behavior
